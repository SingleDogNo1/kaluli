...
// const SERVER_URL = 'http://cy0j.appheader.com';
const SERVER_URL = 'https://wm.armyhorse.com';
...
module.exports = {
	...
	post: function(url, data, successCallback, completeCallback) {
		...
		let header = {
			...
			'appid': '59194a5bb97d90d63b42321b',
			'username': 'appheader'
		};
		...
		wx.request({
			...
			success: function(res) {
				...
				} else if(res.data.errno && res.data.errno != 0) {//请求包含错误代码
					if(10004 == res.data.errno) {
						self.showModal('操作失败', '请允许小程序获取您的用户信息', function() {
							self.reLogin();
						}, false);
					} else {
						console.log('服务请求失败:', res);
						self.showError(res.data.errmsg);
					}
				} else {//请求成功
					wx.hideLoading();
